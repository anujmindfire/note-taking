...
## 1.2 User Personas
To ensure functional specifications address real needs, JotDown features are aligned with two primary user personas:
...
* **JotDown Solution:** Automatic version snapshots, tag-based grouping, and lightning-fast full-text search.
...
* **JotDown Solution:** Secure shareable link generation with custom expiration dates and clean markdown presentation.

---

# 2. Scope Matrix

The scope of JotDown is carefully scoped to maintain a lightweight, highly responsive system architecture.
...
> [!NOTE]
> **Anti-Enumeration Guard:** To prevent attackers from checking registered email addresses, the `/forgot-password` endpoint will always respond with `HTTP 200 Success`, irrespective of whether the email exists in the database.
